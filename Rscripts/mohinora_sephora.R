...
pixel_pastizal <- mohinora_DATA_rTp$values[xy$coord, ] * 1e-4

# ---

BASIS <- drbasis(n=50, q=2)

output_pastizal <- phenopar(x=pixel_pastizal,
...
plot(pixel_pastizal,
     startYear=2000, endYear=2024, frequency=23)
...
xy <- get_timeSeries_byClicking(c(XY$x, XY$y), 
                                df=mohinora_DATA_rTp$coords) #xy$coord = 210

pixel_agro <- mohinora_DATA_rTp$values[xy$coord, ] * 1e-4
...
pixel_ayarin <- mohinora_DATA_rTp$values[xy$coord, ]  * 1e-4

# ---

BASIS <- drbasis(n=50, q=2)

output_ayarin <- phenopar(x=pixel_ayarin,
                          startYear=2000, endYear=2024, 
                          frequency=23, basis=BASIS,
                          distance="dtw_basic", 
                          clusterSize=15, numFreq=3, k=2)
...
pixel_ayarin <- mohinora_DATA_rTp$values[2734, ]  * 1e-4
...
cluster1_ayarin <- phenopar(x=test_ayarin,
                            startYear=2000, endYear=2024, 
                            frequency=23, basis=BASIS,
                            distance="dtw_basic", 
                            series = CLUSTER1,
...
cluster2_ayarin <- phenopar(x=test_ayarin,
...
pixel_pinoEncino <- mohinora_DATA_rTp$values[xy$coord, ]  * 1e-4
...
mohinora_pastizal_rTp <- spRast_valuesCoords(mohinora_pastizal, na_rm = TRUE)
