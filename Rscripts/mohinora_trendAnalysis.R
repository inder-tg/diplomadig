...
# --- Actualizado: Agosto 3, 2023, Agosto 16, 2024
...
pixel_full_ts <- ts(pixel_full, start = c(2000,1), end = c(2023,23),
                    frequency = 23)

plot(pixel_full_ts, ylab="NDVI (sin rescalar)")

pixel_MannKendall <- mk.test(as.numeric(pixel_full) * 1e-4)

pixel_MannKendall$p.value

pixel_SenTheil <- sens.slope(as.numeric(pixel_full) * 1e-4)

pixel_SenTheil$estimates

b_hat <- as.numeric(pixel_SenTheil$estimates)
a_hat <- median( as.numeric(pixel_full) * 1e-4 - b_hat * 1:length(pixel_full) )
...
plot(pixel_full_ts * 1e-4, type="l", col = "gray", ylab = "NDVI")
...
df_pvalue <- matrix(nrow=nrow(mohinora_NDVI_rTp_full$coords), ncol=3)
df_pvalue[,1:2] <- mohinora_NDVI_rTp_full$coords

df_slope <- matrix(nrow=nrow(mohinora_NDVI_rTp_full$coords), ncol=3)
df_slope[,1:2] <- mohinora_NDVI_rTp_full$coords
...
# # ACTION REQUIRED
# Asegurarse de crear folder /TIF/mohinora_trendAnalysis
raster::writeRaster(map_pvalue,
            filename = paste0( getwd(), "/TIF/mohinora_trendAnalysis/pvalueMap"),
            format="GTiff", datatype="FLT4S", overwrite=TRUE)

raster::writeRaster(map_slope,
                    filename = paste0( getwd(), "/TIF/mohinora_trendAnalysis/slopeMap"),
                    format="GTiff", datatype="FLT4S", overwrite=TRUE)
# ---
...
writeRaster(slopeMap,
            filename=paste0( getwd(), "/TIF/mohinora_trendAnalysis/slopeMap_sign"),
            format="GTiff", datatype="FLT4S", overwrite=TRUE)
# --- 
